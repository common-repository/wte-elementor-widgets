...
list($settings, $trip_id, $results_array) = $args;
...
$priceType         = wte_array_get($settings, 'priceType', '2');
...
$position          = wte_array_get($settings, 'loc_position', 'bottom');
...
$enable_slider     = wte_array_get($settings, 'enableSlider', true);
	...
	<div class="wpte-card__wrap">
		<div class="wpte-card__media">
			...
			<figure class="wpte-card__image">
				<?php $add_class = has_post_thumbnail($trip_id) ? '' : 'wpte-card__fallback-img'; ?>
				<a href="<?php echo esc_url(get_the_permalink($trip_id)); ?>" class="<?php echo esc_attr($add_class); ?>">
					<?php
		...
		<div class="wpte-card__content">
						...
						<div class="wpte-card__meta-content">
							...
							if (('days' !== $trip_duration_unit) || ('days' === $trip_duration_unit && $meta->duration['days'] && in_array($set_duration_types, array('both', 'days')))) {
								$duration_label[] = sprintf(
									...
									$results_array['duration'][$trip_duration_unit][0],
									$results_array['duration'][$trip_duration_unit][1]
								);
							}
			...
			if (wte_array_get($settings, 'showTitle', true)) : ?>
				<h2 class="wpte-card__title" itemprop="name">
					<a itemprop="url" href="<?php echo esc_url(get_the_permalink($trip_id)); ?>"><?php echo esc_html( get_the_title( $trip_id ) ); ?></a>
				</h2>
			<?php endif; ?>
			<?php
			...
			<div class="wpte-card__location-wrapper">
				<?php if (wte_array_get($settings, 'showLocation', false)) :
